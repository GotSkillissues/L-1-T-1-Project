...
void collcheck1()
{
	for (int i = 0; i < 60; i++)
	{
		...
		if (i == 3 && b[i].show == false && p[3].used == false)
		{
			p[3].active = true;
			p[3].type = 4;
			if (!p[3].move)
...
void collcheck2()
{
	for (int i = 0; i < 80; i++)
	{
		...
		if (i == 9 && b2[i].show == false && p[0].used == false)
		{
			p[0].active = true;
			p[0].type = 1;
			if (!p[0].move)
		...
		if (i == 19 && b2[i].show == false && p[1].used == false)
		{
			p[1].active = true;
			p[1].type = 2;
			if (!p[1].move)
...
void collcheck3()
{
	...
	for (int i = 0; i < 100; i++)
	{
		if (b3[i].show)
		{
			...
			if (i == 1 && b2[i].show == false && p[0].used == false)
			{
				p[0].active = true;
				p[0].type = 1;
				if (!p[0].move)
			...
			if (i == 13 && b2[i].show == false && p[5].used == false)
			{
				p[5].active = true;
				p[5].type = 6;
				if (!p[5].move)
...
void ballchange()
{
	if (start)
	{
		...
		if (fastball == 1)
		{
			ball_x += (int)(1.45 * ball_dx);
			ball_y += (int)(1.45 * ball_dy);
		}
		else if (slowball == 1)
		{
